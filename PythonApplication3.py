...
def searc(date, name) -> tuple:
    """Return tuple with person's data
    ...
    :return: Zodiak sign, Zodiak element, name
...
@dp.message_handler()
async def processor(message: types.Message) -> bool:
    ...
    :rtype: bool
    :return: True if the supported command, else False

    """
    print(type(message))
    message = types.message.Message()
    print(type(message))
    message.text = "a 25.02.2020\nb 25.02.2020"
    if message.text == "/start":
        await message.answer(texts.Info)
        return True
    else:
        try:
            str1, str2 = message.text.split("\n")
            name, dt = str1.split()
            hum1 = searc([int(x) for x in dt.split(".")], name)
            name, dt = str2.split()
            hum2 = searc([int(x) for x in dt.split(".")], name)
            await comp(hum1, hum2, message)
            return True
        except:
